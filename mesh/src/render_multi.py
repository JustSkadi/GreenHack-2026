...
STATION_NODE_SIZE = 140  # 2× zwykła kulka (70)
STATION_COLOR = "#f59e0b"  # stacja RCB — widoczna na ciemnym tle
RCB_BROADCAST_RUN_INDEX = 1
RCB_PHASE_SLOW = 5  # faza 2: 5× wolniejsza animacja
RCB_MESSAGE = (
...
class RcbBroadcast:
    ...
    active_hops: list[RcbHop] = field(default_factory=list)
...
def _last_delivered_node(route: Route) -> int:
    return route.path[-1] if route.path else route.current_node


def _anchor_packet(route: Route, pkt: PacketAnim) -> None:
    """Pakiet na ostatnim zatwierdzonym węźle — po zerwanej transakcji hopu."""
    node = _last_delivered_node(route)
    pkt.hop_from = node
    pkt.hop_to = node
    pkt.hop_t = 1.0


def _hop_xy(hop_from: int, hop_to: int, hop_t: float, pos: dict) -> tuple[float, float]:
    if hop_t < 1.0 and hop_from in pos and hop_to in pos:
        t = _smoothstep(hop_t)
        x1, y1 = pos[hop_from]
        x2, y2 = pos[hop_to]
...
def _packet_xy(pkt: PacketAnim, pos: dict) -> tuple[float, float]:
    if (
        pkt.hop_t < 1.0
        and pkt.hop_from != pkt.hop_to
        and pkt.hop_from in pos
        and pkt.hop_to in pos
    ):
        return _hop_xy(pkt.hop_from, pkt.hop_to, pkt.hop_t, pos)
    if pkt.route.delivered and pkt.route.destination in pos:
        return pos[pkt.route.destination]
    anchor = _last_delivered_node(pkt.route)
    if anchor in pos:
        return pos[anchor]
    return 0.5, 0.5
...
def render_multi():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Liczba węzłów w grafie (domyślnie 2× więcej niż trening)",
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
    ...
    rcb_hop_frames = max(15, (hop_frames // 8) * RCB_PHASE_SLOW)
    ...
    node_scatter = None
    packet_scatter = None
    rcb_packet_scatter = None
    trail_line = None
    repeater_scatter = None
    station_overlay = None
    title = ax.set_title("", color="#e5e7eb", fontsize=13, pad=12, loc="center")
    ...
    def _rcb_launch_wave() -> bool:
        """Jedna fala — wszystkie przekazy z wave_front startują naraz."""
        if rcb_state is None:
            return False
        rcb_state.active_hops.clear()
        ...
        for src in sorted(rcb_state.wave_front):
            ...
            for n in env.G.neighbors(src):
                if n in rcb_state.received or n in rcb_state.wave_scheduled:
                    continue
                if not env.G.has_edge(src, n):
                    continue
                rcb_state.active_hops.append(RcbHop(hop_from=src, hop_to=n, hop_t=0.0))
                rcb_state.wave_scheduled.add(n)
        return bool(rcb_state.active_hops)

    def _rcb_advance_wave() -> bool:
        """Następna warstwa kaskady — nadawcami są węzły z poprzedniej fali."""
        if rcb_state is None:
            return False
        delivered = set(rcb_state.wave_scheduled) & rcb_state.received
        if not delivered:
            return False
        rcb_state.wave_front = delivered
        rcb_state.wave_num += 1
        return _rcb_launch_wave()

    def _rcb_hops_in_flight() -> bool:
        return bool(rcb_state and any(h.hop_t < 1.0 for h in rcb_state.active_hops))

    def _rcb_try_finish():
        nonlocal runs_completed, simulation_done, last_delivery_s
        if rcb_state is None or rcb_state.counted:
            return
    ...
    def _setup_rcb_run():
        nonlocal rcb_state, endpoints, pending_hop_to, run_transport_s, irl_time_s
        edge_tracker.clear()
    ...
    def _tick_irl_clock():
        nonlocal run_transport_s, irl_time_s
        if not irl_mode or simulation_done:
            return
        if rcb_state is not None or route.active or _is_hopping() or _rcb_hops_in_flight():
            dt = ms_per_frame
            if rcb_state is not None:
                dt = ms_per_frame / RCB_PHASE_SLOW
            run_transport_s += dt
            irl_time_s += dt
    ...
    def _step_rcb():
        ...
        if not route.delivered:
            saved_mobility = env.mobility_step
            env.mobility_step = frame_mobility / RCB_PHASE_SLOW
            env.tick_physics()
            env.mobility_step = saved_mobility
        ...
        if _rcb_hops_in_flight():
            ...
            for hop in rcb_state.active_hops:
                ...
                if prev < 1.0 <= hop.hop_t:
                    rcb_state.received.add(hop.hop_to)
                if hop.hop_t < 1.0:
                    still_active.append(hop)
            rcb_state.active_hops = still_active
            sync_artists()
            return
        ...
        if rcb_state and rcb_state.wave_scheduled:
            advanced = _rcb_advance_wave()
            if advanced and not rcb_state.active_hops:
                _rcb_advance_wave()
            if not rcb_state.active_hops and not advanced:
                _rcb_try_finish()
        elif rcb_state:
            _rcb_try_finish()
    ...
    def step(_frame=0):
        ...
        if _is_hopping():
            ...
            if prev_t < 1.0 <= packet.hop_t and pending_hop_to is not None:
                to_node = pending_hop_to
                pending_hop_to = None
                if irl_mode and not env.hop_edge_ok(route, route.current_node, to_node):
                    env.abort_inflight_hop(route)
                else:
                    env.commit_hop(route, to_node)
                _anchor_packet(route, packet)
    ...
    def _clear_artists():
        nonlocal node_scatter, packet_scatter, rcb_packet_scatter, trail_line, repeater_scatter
        nonlocal station_overlay
        edge_tracker.clear()
        if packet_scatter is not None:
            packet_scatter.remove()
            packet_scatter = None
        if rcb_packet_scatter is not None:
            rcb_packet_scatter.remove()
            rcb_packet_scatter = None
        if trail_line is not None:
            trail_line.remove()
            trail_line = None
        ...
        if repeater_scatter is not None:
            ...
            repeater_scatter = None
        if station_overlay is not None:
            station_overlay.remove()
            station_overlay = None
    ...
    def sync_artists():
        ...
        for n in env.G.nodes:
            ...
            ys.append(pos[n][1])
            if is_station:
                sizes.append(STATION_NODE_SIZE)
                colors.append(STATION_COLOR)
            elif got_rcb:
                sizes.append(100)
        ...
        if route.is_rcb_broadcast and rcb_state is not None:
            for n in env.G.nodes:
                if env.G.nodes[n].get("is_112") and n in pos:
                    station_node = n
                    break
        if station_node is not None:
            sx, sy = pos[station_node]
            if station_overlay is None:
                station_overlay = ax.scatter(
                    [sx], [sy], s=STATION_NODE_SIZE, c=STATION_COLOR,
                    zorder=7, edgecolors="#ffffff", linewidths=1.4,
                )
            else:
                station_overlay.set_offsets(np.array([[sx, sy]]))
        elif station_overlay is not None:
            station_overlay.remove()
            station_overlay = None

        tx, ty = _trail_points(route, packet, pos)
        if tx and not route.is_rcb_broadcast:
        ...
        if route.is_rcb_broadcast and rcb_state is not None and rcb_state.active_hops:
            rpx, rpy = [], []
            for hop in rcb_state.active_hops:
                if hop.hop_t < 1.0:
                    x, y = _hop_xy(hop.hop_from, hop.hop_to, hop.hop_t, pos)
                    rpx.append(x)
                    rpy.append(y)
            if rpx:
                if rcb_packet_scatter is None:
                    rcb_packet_scatter = ax.scatter(
                        rpx, rpy, s=160, c=route.color,
                        edgecolors="#fff", linewidths=1.2, zorder=5,
                    )
                else:
                    rcb_packet_scatter.set_offsets(np.column_stack([rpx, rpy]))
            elif rcb_packet_scatter is not None:
                rcb_packet_scatter.set_offsets(np.empty((0, 2)))
            if packet_scatter is not None:
                packet_scatter.set_offsets(np.empty((0, 2)))
        else:
            if rcb_packet_scatter is not None:
                rcb_packet_scatter.set_offsets(np.empty((0, 2)))
        ...
        title.set_text(_format_sim_time(irl_time_s))
    ...
    if args.output:
        ...
        try:
            with writer.saving(fig, args.output, dpi=args.dpi):
                i = 0
                cap = max(args.frames, 8000 * max_runs)
                while i < cap:
                    step(i)
                    writer.grab_frame()
                    i += 1
